...
local function test(name, aliases, callback, targetFunctionBase, index)
    running = running + 1
    local targetFunction = targetFunctionBase
    if index and targetFunctionBase then
        targetFunction = targetFunctionBase[index]
    end
    ...
    task.spawn(function()
        ...
        elseif not getGlobal(name) then
            ...
            if closureType then
                if success then
                    passes = passes + 1
                    print("✅ " .. name .. (message and " • " .. message or "") .. " - " .. closureType)
                else
                    fails = fails + 1
                    warn("⛔ " .. name .. " failed: " .. message .. " - " .. closureType)
                end
            else
                if success then
                    passes = passes + 1
...
task.defer(function()
    ...
    print("\n")
    print("Summary")
    print("✅ Completed all the test function with a " .. rate .. "% success rate (" .. outOf .. ")")
    print("⛔ " .. fails .. " Tests failed")
    print("⚠️ " .. undefined .. " Globals are missing aliases")
    print("ℹ️ " .. cclosureamount .. "% of functions are C closures")
    print("ℹ️ " .. lclosureamount .. "% of functions are Lua closures")
    print("Version 022625162 - Initial | Credit to: norby.dev, vxsty, and sharpcystals")
end)
